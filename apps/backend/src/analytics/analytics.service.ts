...
export class AnalyticsService {
  ...
  async spendByMonth(userId: string) {
    ...
    const results = await this.prisma.$queryRawUnsafe<
      { year: number; month: number; total: number; currency: string }[]
    >(
      `
  ...
  async categoryTrends(userId: string, months: number = 6) {
    const cardIds = await this.cardsService.getAccessibleCardIds(userId);
    if (!cardIds.length) return [];
    ...
    const trends: Record<string, { month: string; amount: number }[]> = {};
    results.forEach((r) => {
  ...
  async topCategories(userId: string, limit: number = 5) {
    const cardIds = await this.cardsService.getAccessibleCardIds(userId);
    if (!cardIds.length) return [];
  ...
  async spendingInsights(userId: string) {
    const cardIds = await this.cardsService.getAccessibleCardIds(userId);
    if (!cardIds.length) {
      return {
        totalSpend: 0,
    ...
    const monthlyChange =
      lastMonthStats._sum.amount && lastMonthStats._sum.amount > 0
